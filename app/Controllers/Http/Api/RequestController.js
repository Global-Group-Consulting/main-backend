...
class RequestController {
  ...
  async create ({ request, response, auth }) {
    ...
    // Se la richiesta è una riscossione capitale, aggiungo il movimento di riscossione temporaneo
    if (incomingData.type === RequestTypes.RISC_CAPITALE) {
  ...
  async reject ({ request, params, response }) {
    ...
    if (foundedRequest.type === RequestTypes.RISC_CAPITALE) {
      const movement = await MovementModel.where({
        "requestId": castToObjectId(foundedRequest._id),
        approved: false
      }).first()

      await updateNextMovements(movement, true)
