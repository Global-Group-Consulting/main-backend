...
const updateNextMovements = require('./movements/updateNextMovements')
...
class MovementController {
  ...
  async delete ({ request, params, auth }) {
    ...
    await updateNextMovements(movementRef, true)
  ...
  async update({ request, params, auth }) {
    ...
    if (!auth.user.superAdmin) {
      throw new MovementErrorException('You don\'t have the permission to update this movement.')
    }

    const movement = await MovementModel.findOrFail(movementId)

    movement.appendToHistory( auth.user._id, {
      [field]: {
        old: movement[field],
        new: amount,
    ...
    try {
      const updatedMovements = await updateNextMovements(movement)

      updatedMovements.unshift(movement)

      return updatedMovements
    } catch (e) {
      let message = 'Can\'t update movement.'

      if (e.movement) {
        message = `Non posso aggiornare il movimento ${e.movement._id.toString()} per il seguente motivo: ${e.error.message}`
      }

      throw new MovementErrorException(message)
    }
  }
